...
export default function AccountBookingsSection({
  ...
  const activeBookings = bookings.filter((booking) => !hasBookingEnded(booking));
  const pastBookings = bookings.filter((booking) => hasBookingEnded(booking));
        ...
        <div className="mt-8 border-t border-white/10 pt-8">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-7 text-[#94A3B8]">
                Finished games stay here for reference after their slot has
                ended.
              </p>
            </div>

            <div className="rounded-[20px] border border-white/10 bg-white/[0.04] px-4 py-3">
              <p className="text-sm text-[#94A3B8]">Completed bookings</p>
              <p className="mt-1 text-lg font-semibold text-white">
                {pastBookings.length}
              </p>
            </div>
          ...
          <div className="space-y-4">
            {pastBookings.map((booking) => (
              <div
                key={booking.id}
                className="rounded-[26px] border border-white/10 bg-white/[0.03] p-5 opacity-95"
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs text-[#C9D2DC]">
                ...
                <p className="mt-4 text-sm text-[#94A3B8]">
                  This game has already finished, so it now appears in your
                  booking history.
                </p>
              </div>
            ))}
