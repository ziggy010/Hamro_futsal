...
export default function AdminDashboard() {
  ...
  const expiryNowGames = useMemo(() => {
    ...
    return activeOpenGames.filter((game) => {
      const cutoff = getEffectiveCutoffTime(game);
      return (
  ...
  const openGamesWatchlist = useMemo(() => {
    return [...activeOpenGames]
      .sort(
        (a, b) =>
          getEffectiveCutoffTime(a).getTime() -
          getEffectiveCutoffTime(b).getTime(),
      )
      .slice(0, 6);
  }, [activeOpenGames]);
      ...
      <section className="container py-8 md:py-12">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            {
              title: "Needs Expiry Now",
              value: String(expiryNowGames.length),
              meta:
                expiryNowGames.length > 0
                  ? "Underfilled games past cutoff"
                  : "No overdue open games",
              icon: TriangleAlert,
            },
            {
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-[22px] border border-white/10 bg-white/[0.04] p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-6 text-[#94A3B8]">
                        ...
                        <br />
                        Need expiry now: {expiryNowGames.length}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid gap-2 text-sm text-[#D7DEE7]">
                    ...
                    <div className="rounded-[16px] border border-white/10 bg-[#121821] px-3 py-3">
                      Expire underfilled open games once the 4-hour cutoff has passed so the slot can be sold again.
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 space-y-3">
                {loading ? (
                  ...
                  openGamesWatchlist.map((game) => {
                    ...
                    const urgent =
                      effectiveCutoff.getTime() - Date.now() <=
                        2 * 60 * 60 * 1000 &&
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-[22px] border p-4 ${
                          needsExpiryNow
                            ? "border-[#7A2424] bg-[rgba(52,18,20,0.72)] shadow-[0_0_0_1px_rgba(255,120,120,0.08)]"
                        ...
                        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {needsExpiryNow && (
                                <span className="inline-flex rounded-full border border-[#A63A3A] bg-[#34161A] px-3 py-1 text-xs text-[#FFB4B4]">
                                  Needs expiry now
                                </span>
                              )}
                              {urgent && (
                          ...
                          <div className="rounded-[18px] border border-white/10 bg-[#121821] px-4 py-3 text-right">
                            <p className="text-xs text-[#94A3B8]">
                              {needsExpiryNow ? "Action" : "Cutoff"}
                            </p>
                            <p
                              className={`mt-1 text-sm font-medium ${
                                needsExpiryNow ? "text-[#FFB4B4]" : "text-white"
                              }`}
                            >
                              {needsExpiryNow
                                ? "Expire and reopen slot"
                                : timeUntil(effectiveCutoff.toISOString())}
                            </p>
                          </div>
                        </div>
