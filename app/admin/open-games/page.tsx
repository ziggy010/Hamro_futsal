...
export default function AdminOpenGamesPage() {
  ...
  const canExpireNow = (game: OpenGameApi) => {
    const cutoff = getOpenGameCutoffTime(
      new Date(game.booking.bookingDate),
      game.booking.slots,
    );
    return (
  ...
  const runExpireSweep = async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Failed to expire open games");
      }

      setActionMessage(
        `Expired ${data?.expiredCount ?? 0} open game${data?.expiredCount === 1 ? "" : "s"}.`,
      );
      await fetchGames();
      setSelectedGame(null);
    } catch (fetchError: unknown) {
      setActionError(getErrorMessage(fetchError, "Failed to expire open games"));
    } finally {
      setProcessingId("");
    }
  };
  ...
  const handleCancel = async (openGameId: string) => {
    ...
    try {
      ...
      await fetchGames();
      if (selectedGame?.id === openGameId) {
        setSelectedGame(null);
      }
    } catch (fetchError: unknown) {
      setActionError(getErrorMessage(fetchError, "Error cancelling game"));
    } finally {
      setProcessingId("");
    }
  };
        ...
        {actionError && (
          <div className="mb-6">
            <StatePanel
              ...
              title="The open-game action did not complete"
              text={actionError}
                ...
                activeGames.map((game) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {canExpireNow(game) && (
                        <div className="mt-3 rounded-[16px] border border-[#7A2424] bg-[#34161A] px-3 py-3 text-sm text-[#FFB4B4]">
                          This game passed the 4-hour cutoff without enough players and can be expired now.
                        </div>
                      )}
